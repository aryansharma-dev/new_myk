...
const SubAdminOrders = () => {
  ...
  useEffect(() => {
    document.title = 'Orders - Sub Admin';
    ...
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by order id or customer" className="px-3 py-2 border rounded" />
          ...
          <button onClick={fetchOrders} className="px-4 py-2 bg-black text-white rounded">Filter</button>
        </div>
      </div>
            ...
            <tbody>
              {orders.map((o) => (
                <tr key={o._id} className="border-t hover:bg-gray-50">
                  <td className="px-4 py-3 font-mono text-sm">{String(o._id).slice(-8)}</td>
                  <td className="px-4 py-3">{o.user?.name || o.user?.email || 'Customer'}</td>
                  <td className="px-4 py-3">{(o.cartItems || []).length}</td>
                  <td className="px-4 py-3">₹{o.totalAmount || o.amount}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded text-xs ${STATUS_COLORS[o.status] || 'bg-gray-100 text-gray-800'}`}>
                      {o.status}
                    </span>
                  </td>
                  <td className="px-4 py-3">{new Date(o.createdAt).toLocaleString()}</td>
                  <td className="px-4 py-3"><button onClick={() => openOrder(o)} className="px-3 py-1 border rounded">Details</button></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded shadow max-w-3xl w-full p-6 overflow-auto max-h-[90vh]">
            ...
            <div className="space-y-3">
              <div className="text-sm text-gray-600">Order ID: <span className="font-mono">{selected._id}</span></div>
              <div className="text-sm">Customer: {selected.user?.name} ({selected.user?.email})</div>
              <div className="text-sm">Status: <strong>{selected.status}</strong></div>
              <div className="mt-4">
                <h4 className="font-medium">Items</h4>
                <div className="mt-2 grid gap-2">
                  {(selected.cartItems || []).map((it, idx) => (
                    <div key={idx} className="flex items-center gap-3 border rounded p-2">
                      <img src={it.product?.images?.[0] || '/placeholder.png'} className="w-14 h-14 object-cover rounded" alt="" />
                      <div>
                        <div className="font-medium">{it.name}</div>
                        <div className="text-sm text-gray-600">Qty: {it.quantity} • Size: {it.size}</div>
                      </div>
                      <div className="ml-auto font-semibold">₹{it.price}</div>
                    </div>
                  ))}
                </div>
              </div>
